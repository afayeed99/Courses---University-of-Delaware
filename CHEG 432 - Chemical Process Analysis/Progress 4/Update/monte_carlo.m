...
%%
prod0 = 170; % MM kg/yr meoac production
%prod = norminv(prob,prod0,0.1*prod0);
prod = norminv(prob,prod0,0.1);

vga0 = 54; % specify vga values $MM
%vga = norminv(prob,vga0,0.1*vga0);
vga = norminv(prob,vga0,0.1);

% specify at the very beginning
meoh_flow0 = 699.3;
co_flow0 = 300;
cat_cost = 62206; % $ 
%meoh_flow = norminv(prob,meoh_flow0,0.02*meoh_flow0);
%co_flow = norminv(prob,co_flow0,0.02*co_flow0);
meoh_flow = norminv(prob,meoh_flow0,0.02);
co_flow = norminv(prob,co_flow0,0.02);
...
% steam_qpu = norminv(prob,steam_qpu0,0.1*steam_qpu0);
steam_qpu = norminv(prob,steam_qpu0,0.1);
...
% num_op = norminv(prob,num_op0,0.1*num_op0);
num_op = norminv(prob,num_op0,0.1);
...
%% correct but depends on the size of rand number
% [sorted_tp,idx] = sort(tp);
% sorted_prob = prob(idx);
% % figure()
% tp_pdf(1) = sorted_tp(1);
% prob_pdf(1) = sorted_prob(1);
% for i = 2:length(prob)
%     prob_pdf(i) = sorted_prob(i) - sorted_prob(i-1);
% end
% 
% figure()
% plot(sorted_tp,prob_pdf,'-')
%%
% figure(2)
% 
% [p,x] = hist(tp); plot(x,p/sum(p)); %PDF
% 
% % %%
% % 
% % [sorted_tp,idx] = sort(tp);
...
%     dx(i) = sorted_tp(i+1) - sorted_tp(i);
%     dy(i) = prob(i+1) - prob(i);
% end
% 
% pdf = dx./dy;
% plot(sorted_tp(1:end-1,:),pdf')
