...
% % OVERALL STAGE LEVEL 1 ANALYSIS
% ca1_i_new = lambda_val.*(caf0_ii - ca2_ii_goal) + caf1;
% 
% guess_conc = ones(size(ca1_i_new)).*2;
% opt = optimset('Display','notify');
% %%
% for i = 1:length(ca1_i_new(1,:)) % going through each column
%     % F = concA(x,qi,qii,caf0_ii,ca1_i,caf_i,ca2_ii)
%     [ca2_i_new(:,i),ca1_ii_new(:,i)] = fsolve(@concA,guess_conc(:,i),opt,q1list(:,i),q2,caf0_ii,...
%         ca1_i_new(:,i),caf1,ca2_ii_goal);
% end
%     
% %%
% i = 1;
% conc_array = fsolve(@concA,guess_conc(i),opt,q1list(i),q2,caf0_ii,...
%         ca1_i_new(i),caf1,ca2_ii_goal);
...
avgc_I_II = (ca1_i_val - M.*ca2_ii_goal); % g/L

% avgc_I_II = -((ca1_i_val - M.*caf0_ii) - (caf1 - M.*ca2_ii_val))./...
%     (log((ca1_i_val - M.*caf0_ii)./(caf1 - M.*ca2_ii_val)));
...
area = mload./60./km./avgc_I_II./1e3; % m^2
...
v_i = 1./6.*area.*dlist.*1000; 
v_ii = v_i.*q1list./q2; 

v_combine = v_i + v_ii;

total_tcecost = v_i.*rho1.*TCE_cost; % $

masstotal = rho1.*v_i + rho2.*v_ii;

total_powercost = powerlist.*masstotal.*power_cost./1000.*24; % $ .*tau./3600

totalc = total_tcecost + total_powercost; % correct dimensions
